...
_TK_debt = " หนี้ "
_TK_notok = " ไม่โอเค "
_TK_comp1 = " ไม่ได้เรื่อง "
_TK_comp = " ด่า "
_TK_comp2 = " ไม่รู้ "
_TK_better = " ดีกว่านี้ "
_TK_why = " ทำไม "
_TK_fix = " แก้ปัญหา "
_TK_snot = " ไม่ควร "
_TK_s = " ควร "
...
RULES = [
        ('(ลค\.|ผู้ใช้บริการ|คนมาใช้บริการ|ลุกค้า)', 'ลูกค้า'),
        ('(เฟิสช้อย|เฟิร์สช้อย|เฟิสช้อยส์|เฟิร์สช้อยส์|kfc|krungsri( )*first( )*choice|first( )*choice)',_TK_fc),
        ('(กรุงศรีฯ|ศูนย์บัตร|krungsri)', _TK_bay),
        ('(ธนาคาร|ธ\.|แบงค์|แบงก์|แบง|แบ้ง|bank)', _TK_bank),
        ('(ยกเลิกบัตร|ปิดบัตร|cancle( )*บัตร|ไม่เอาบัตร)', _TK_cancle),
        ('(พนักงาน|เจ้าหน้าที่|cc|call( )*center|cs|agent|ลูกน้อง|พนง\.|พนง|คอลเซ็นเตอร์|คอลเซ็นเตอ)', _TK_staff),
        ('(บัตรเครดิต|บัตร)', _TK_card),
        ('(โปรโมชั่น|promotion)', _TK_pro),
        ('(email|อีเมลล์|อีเมล|เมลล์|เมล)', _TK_mail),
        ('(point|แต้ม)', _TK_point),
        ('(ตอแหล|ห่าเหว|ห่า|ระยำ|บ้าบอ|ปัญญาอ่อน|เหี้ย|ควาย|โคตรห่วย|ห่วยแตก|ห่วย|โง่|กวนประสาท|ประสาท|หุบปาก|งี่เง่า|งี้เง่า|กวนตีน|เฮงซวย|ไม่มีปัญญา|กระจอก|กวน|บ้า)', _TK_rude),
        ('(หลอกลวง|โดนหลอก|หลอก|เกินจริง)', _TK_spoof),
        ('(เป็นหนี้|หนี้)', _TK_debt),
        ('(ไม่โอเคร|ไม่โอเค|ไม่( )*ok|ไม่ได้)', _TK_notok),
        ('(ไม่รู้เรื่อง|ไม่ค่อยรู้เรื่อง|ไม่ได้เรื่อง)', _TK_comp1),
        ('(complain|ตำหนิ|ด่า)', _TK_comp),
        ('(ไม่ทราบ|ไม่รู้)', _TK_comp2),
        ('(ดีมากกว่านี้|ดีกว่านี้|ดีขึ้นกว่านี้)', _TK_better),
        ('(สาเหตุอะไร|เพราะอะไร|ทำไม|ด้วยสาเหตุอะไร)', _TK_why),
        ('(ไม่ควร|ไม่เหมาะสม|ไม่เหมาะ)', _TK_snot),
        ('(น่าจะ|ควรจะ)', _TK_s),
        ('(อนุญาต|อนุญาติ)', 'อนุญาต'), # normal typo
        ('(ขออภัย|ขอโทษ)', _TK_sorry),
        ('(แก้ไขปัญหา|แก้ปัญหา|ปรับปรุง)', _TK_fix),
        ('(โมโห|โกรธ|ไม่ชอบ|โกรธา)', _TK_angry),
        ('(ไม่มีความจำเป็น|ไม่ต้อง)', _TK_notn),
        ('(เคลีย|clear)', 'เคลียร์'),
        ('(ไม่ได้สนใจ|ไม่สนใจ|ไม่ได้แคร์)', _TK_ignore),
        ('(ขอบใจ|ขอบคุณ|ขอบพระคุณ)', _TK_thank),
        ('(%|เปอร์เซ็น|percent|เปอร์เซน|เปอเซน|เปอเซ็น|เปอเซ็นต์|เปอเซนต์)', 'เปอร์เซ็นต์'), # normal typo
        ('(บริการดี|บริการเยี่ยม|ช่วยเหลือดี)', _TK_compli),
        ('(กระจ่าง|กระจ่างแจ้ง|ชัดเจน)', _TK_clear2),#จ้า is not used because it is too general
        ('(คร่ะ|ขร๊|จร้า|คร๊ะ|ฮัฟ|ค่พ|ค่ะ|คับ|ครับ|คร้าบ|คร้า|ค๊าบ|ข่ะ|ขรั่บ|คร๊าบ|ขรั๊บ|ฮะ|งับ|จ๊ะ|จ่ะ|จ้ะ|จร่ะ|อ่ะ|คร๊)', _TK_POLITE),
        ('ๆ+', _TK_WREP2),
        ('(โอเคร๊|โอเคร|โอเค๊|โอวเคร|โอวเช|โอเช|โอเคร|ok|okay|โอเค)', _TK_OK),
        ('\.{2,}', _TK_DOTS),
        (r'["/?!:;]', '')
    ]
...
def custom_norm(text: str) -> str:
    """
        Replace all the provided patterns with the provided tokens in RULES
    """
    for i in RULES:
        text = re.sub(i[0], i[1], text)
    return text

def replace_url(text: str) -> str:#from pythainlp
    """
...
def replace_rep_after(text: str) -> str:#from pythainlp
    """
...
def replace_wrep_post(toks: Collection[str]) -> List[str]:#from pythainlp
    """
...
def rm_brackets(text: str) -> str:#from pythainlp
...
def lowercase_all(toks: Collection[str]) -> List[str]:#from pythainlp
    """
...
def remove_space(toks: Collection[str]) -> List[str]:#from pythainlp
    """
